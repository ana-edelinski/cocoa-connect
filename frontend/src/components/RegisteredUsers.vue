...
const filteredOrders = computed(() => {
  return orders.value.filter(order => {
    const matchesFactoryName = order.factory.name.toLowerCase().includes(searchCriteria.value.factoryName.toLowerCase());
    const matchesPrice = (searchCriteria.value.minPrice === '' || order.price >= searchCriteria.value.minPrice) && (searchCriteria.value.maxPrice === '' || order.price <= searchCriteria.value.maxPrice);
    const matchesDate = (searchCriteria.value.startDate === '' || new Date(order.date) >= new Date(searchCriteria.value.startDate)) && (searchCriteria.value.endDate === '' || new Date(order.date) <= new Date(searchCriteria.value.endDate));
    return matchesFactoryName && matchesPrice && matchesDate;
  });
});
